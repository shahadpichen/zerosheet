...
// Local development keeps the historical repository-root default. The
// production one-shot container points this at a mounted output directory so
// only the two non-secret immutable identifiers cross the container boundary.
const generatedEnvironmentPath = resolve(
  process.env.OPENFGA_STATE_FILE ?? resolve(process.cwd(), ".env.openfga"),
);
